...
export interface SchemaFieldItem {
  type: string;
  description: string;
  /** For nested objects/arrays within items */
...
export const STUDY_TYPES: Record<StudyTypeId, StudyTypeConfig> = {
  ...
  exegetical: {
    ...
    levels: {
      1: {
        schema: {
          what_it_says: { type: "string", description: "Simple paraphrase in a kid's own words, max 2 sentences" },
      ...
      2: {
        schema: {
          ...
          the_words: { type: "object[]", description: "2-3 key words explained", items: { word: { type: "string", description: "The word in English" }, original: { type: "string", description: "Greek/Hebrew transliteration + Strong's" }, real_meaning: { type: "string", description: "What it really meant — no Sunday School answer" } } },
        ...
        promptInstructions: `LEVEL 2 EXEGETICAL: Explain to a 14-18 year old. Show why the original language matters — not because it's academic, but because the English sometimes hides what's actually there. Address identity, pressure, anxiety, or purpose. No clichés.`,
      },
      3: {
        schema: {
      ...
      4: {
        schema: {
          ...
          intertextual_map: { type: "object[]", description: "Cross-references with analysis", items: { ref: { type: "string", description: "Reference" }, relationship: { type: "string", description: "Type: quotation, allusion, echo, thematic" }, exegetical_significance: { type: "string", description: "What this connection proves" } } },
  ...
  thematic: {
    ...
    levels: {
      1: {
        ...
        promptInstructions: `LEVEL 1 THEMATIC: Trace a big Bible idea for a 10-year-old. Use the "First Mention" idea simply. Connect everything to Jesus. Give one concrete action.`,
      },
      2: {
        schema: {
  ...
  character: {
    ...
    levels: {
      ...
      3: {
        schema: {
          biographical_arc: { type: "object", description: "Major life events and covenants", items: { timeline: { type: "string[]", description: "Chronological major events" }, covenants: { type: "string[]", description: "Any covenants God made with them" } } },
    ...
    levels: {
      ...
      4: {
        schema: {
          ...
          homiletical_strategies: { type: "object[]", description: "Multiple preaching approaches", items: { approach: { type: "string", description: "Named approach (expository, thematic, redemptive-historical)" }, big_idea: { type: "string", description: "The big idea from this angle" }, outline: { type: "string", description: "Sermon outline" }, strength: { type: "string", description: "What this approach highlights" }, weakness: { type: "string", description: "What it might miss" } } },
    ...
    levels: {
      1: {
        schema: {
          ...
          what_life_was_like: { type: "string", description: "What daily life was like — food, clothes, houses, jobs" },
    ...
    levels: {
      ...
      4: {
        schema: {
          ...
          oratio: { type: "object", description: "ACTS prayer", items: { adoration: { type: "string", description: "Adoration based on the passage" }, confession: { type: "string", description: "Confession prompted by the passage" }, thanksgiving: { type: "string", description: "Thanksgiving from the passage" }, supplication: { type: "string", description: "Supplication flowing from the passage" } } },
          ...
          historical_devotional_voices: { type: "object[]", description: "Historical devotional insights", items: { figure: { type: "string", description: "Historical figure (e.g., Spurgeon, Augustine)" }, insight: { type: "string", description: "Their insight on this passage" } } },
        },
    ...
    levels: {
      1: {
        schema: {
          ...
          handshake_with_god: { type: "string", description: "A covenant is like a special handshake — God makes a promise and asks us to trust Him. Explain it simply." },
...
export function getStudyTypeLevel(id: StudyTypeId, level: number): StudyTypeLevel {
  return STUDY_TYPES[id].levels[level] || STUDY_TYPES[id].levels[3];
}

export function buildStudyTypeSchema(studyType: StudyTypeId, level: number): string {
