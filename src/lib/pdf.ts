...
export function downloadStudyPDF(study: StudyResponse): void {
  ...
  function addSection(icon: string, title: string): void {
    ...
    doc.text(`${cleanIcon}  ${title}`, MARGIN, y);
    y += 8;
  }
...
// ── Unicode sanitizer for PDF ────────────────────────────────────
// jsPDF's Helvetica font doesn't support Unicode emoji, smart quotes,
// dashes, or other special characters. Strip/replace them.
